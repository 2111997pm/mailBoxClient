...
const Inbox = () => {
  ...
  const clickEmailHanler = async (item) => {
    ...
    try { 
      const resEmail = await fetch(
        `https://mailboxclient-9c262-default-rtdb.firebaseio.com/${email}/recievedEmails/${item[0]}.json`,
        {
          method: "PUT",
          body: JSON.stringify({
            id: item[1].id,
  ...
  const clickDeleteHandler = async (deleteItem) => {
    ...
    const email = auth.email.replace(/[.@]/g, "");
    try {
        const resDlt = await fetch(`https://mailboxclient-9c262-default-rtdb.firebaseio.com/${email}/recievedEmails/${deleteItem[0]}.json`,{
            method: 'DELETE'
        })
        if(!resDlt.ok){
